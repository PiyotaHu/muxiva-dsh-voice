  ...
  factory: (require) => {
    ...
    class SentenceBuffer {
      constructor(send) {
        this.send = send
        this.value = ''
        this.minimumChunkChars = 36
        this.maximumChunkChars = 120
      }
      push(text) {
        this.value += text
        while (this.value.length >= this.minimumChunkChars) {
          const boundary = /[。！？!?；;：:\n]|\.(?:\s|$)/g
          let end = 0
          let match
          while ((match = boundary.exec(this.value)) !== null) {
            const candidate = match.index + match[0].length
            if (candidate >= this.minimumChunkChars && candidate <= this.maximumChunkChars) {
              end = candidate
              break
            }
          }
          if (end === 0 && this.value.length < this.maximumChunkChars) return
          if (end === 0) {
            const window = this.value.slice(0, this.maximumChunkChars)
            const soft = Math.max(window.lastIndexOf('，'), window.lastIndexOf(','), window.lastIndexOf(' '))
            end = soft >= this.minimumChunkChars ? soft + 1 : this.maximumChunkChars
          }
