  ...
  factory: (require) => {
    ...
    class SentenceBuffer {
      constructor(send) {
        this.send = send
        this.value = ''
        this.earlyChunkSent = false
        this.minimumEarlyChars = 48
        this.maximumEarlyChars = 96
      }
      push(text) {
        this.value += text
        if (this.earlyChunkSent || this.value.length < this.minimumEarlyChars) return
        const boundary = /[。！？!?；;：:\n]|\.(?:\s|$)/g
        let end = 0
        let match
        while ((match = boundary.exec(this.value)) !== null) {
          const candidate = match.index + match[0].length
          if (candidate >= this.minimumEarlyChars && candidate <= this.maximumEarlyChars) {
            end = candidate
            break
          }
        }
        if (end === 0 && this.value.length < this.maximumEarlyChars) return
        if (end === 0) {
          const window = this.value.slice(0, this.maximumEarlyChars)
          const soft = Math.max(window.lastIndexOf('，'), window.lastIndexOf(','), window.lastIndexOf(' '))
          end = soft >= this.minimumEarlyChars ? soft + 1 : this.maximumEarlyChars
        }
        const ready = this.value.slice(0, end).trim()
        this.value = this.value.slice(end)
        if (ready) {
          this.earlyChunkSent = true
          this.send('agent.delta', { text: ready })
        }
      }
      flush() {
      ...
      clear() { this.value = ''; this.earlyChunkSent = false }
    }
    ...
    if (typeof process !== 'undefined' && process.env?.NODE_ENV === 'test') {
      exports.__testing = Object.freeze({ SentenceBuffer })
    }
