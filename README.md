...
The reliability-first defaults wait for 2 seconds of continuous silence before emitting ASR Final and use a `0.75` VAD threshold with a 350 ms minimum-speech gate. SenseVoice Finals admit only Chinese and English speech with meaningful text, rejecting non-speech events and Japanese/Korean noise hallucinations. Qwen3-TTS uses `Serena` with a fixed calm prosody profile and conservative sampling; browser-side phrase buffering avoids restarting the voice for every tiny sentence, while bounded real-time PCM pacing prevents the playback queue from growing without limit.
